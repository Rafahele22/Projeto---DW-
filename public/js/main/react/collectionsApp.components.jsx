...
function SaveOption({ label }) {
  ...
  return (
    <a href="#" className={"save-option" + (selected ? " selected-option" : "")} data-type={label.toLowerCase()} onClick={(e) => { e.preventDefault(); e.stopPropagation(); setSelected((v) => !v); }}>
    ...
    >
      ...
      <section style={{ display: 'flex', gap: '0.2vw' }}>
          <a 
            ...
            onClick={(e) => { e.preventDefault(); onSetMode('grid'); }}
          >
            {}
            <img 
              src={activeMode === 'grid' ? "../assets/imgs/grid_selected.svg" : "../assets/imgs/grid.svg"} 
          ...
          <a 
            ...
            onClick={(e) => { e.preventDefault(); onSetMode('list'); }}
          >
            <img 
              src={activeMode === 'list' ? "../assets/imgs/list_selected.svg" : "../assets/imgs/list.svg"} 
...
function ListItem({ font, forceFavSelected = false, globalText, setGlobalText, onOpenFont, openSaveId, setOpenSaveId }) {
  ...
  return (
    <div
      ...
      onClick={(e) => {
        if (e.target.closest("a") || e.target.closest("input") || e.target.closest(".save_list") || e.target.closest(".range-container") || e.target.closest('h1[contenteditable="true"]')) return;
        onOpenFont?.(font);
      }}
    >
      ...
      <h1
        ...
        ref={editableRef}
        style={{ fontFamily: `'${font?._id}-font'`, lineHeight: "4.5vw", wordWrap: "break-word", overflowWrap: "break-word", whiteSpace: "normal", outline: "none" }}
        onClick={(e) => e.stopPropagation()}
        onInput={(e) => {
          const visualText = e.currentTarget.innerText;
          const isCapsLockOn = !!window.__collectionsIsCapsLockOn;
          if (!hasAllCaps) { setGlobalText?.(visualText); return; }
          if (!isCapsLockOn) { setGlobalText?.(visualText.toLowerCase()); } else { setGlobalText?.(visualText); }
        }}
      />
    </div>
  );
